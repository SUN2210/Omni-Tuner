...
class MixedDatasetSampler(Sampler):
    ...
    def __iter__(self):
        ...
        # 按batch组织索引
        batch_indices = []
        debug_batches = []  # 用于调试的batch信息
        
        for batch_idx in range(self.num_batches):
            batch = []
            batch_info = {'batch_idx': batch_idx, 'samples': []}
            ...
            for dataset_idx, ratio in enumerate(self.dataset_ratios):
                ...
                if end_idx <= len(dataset_indices):
                    current_indices = dataset_indices[start_idx:end_idx]
                    batch.extend(current_indices)
                    
                    # 记录调试信息
                    for idx in current_indices:
                        batch_info['samples'].append({
                            'global_idx': idx,
                    ...
                    # 处理最后一个batch可能不足的情况
                    current_indices = dataset_indices[start_idx:]
                    batch.extend(current_indices)
            ...
            if len(batch) > 0:
                ...
                shuffled_indices = torch.randperm(len(batch), generator=g)
                shuffled_batch = batch_tensor[shuffled_indices].tolist()
                
                # 更新调试信息的顺序
                original_samples = batch_info['samples'].copy()
                batch_info['samples'] = [original_samples[i] for i in shuffled_indices.tolist()]
                batch_info['shuffled_global_indices'] = shuffled_batch
                
                batch_indices.extend(shuffled_batch)
                debug_batches.append(batch_info)
        
        # ===== 调试信息：输出前5个batch的详细信息 =====
        print("\n" + "="*80)
        print("🔍 BATCH COMPOSITION DEBUG - First 5 Batches")
        print("="*80)
        
        for i in range(min(5, len(debug_batches))):
            batch_info = debug_batches[i]
            ...
            for sample in batch_info['samples']:
                ...
                dataset_counts[dataset_idx] = dataset_counts.get(dataset_idx, 0) + 1
            
            print(f"   📊 Composition: {dataset_counts}")
            print(f"   🎯 Target ratio: {dict(enumerate(self.dataset_ratios))}")
            
            # 显示每个样本的详细信息
            for j, sample in enumerate(batch_info['samples']):
                dataset_name = f"Dataset_{sample['dataset_idx']}"
                
                # 尝试获取图片文件名
                img_info = ""
                try:
                    # 获取对应的子数据集
                    sub_dataset = self.actual_dataset.datasets[sample['dataset_idx']]
                    local_idx = sample['local_idx']
                    
                    # 处理RepeatDataset的情况
                    if hasattr(sub_dataset, 'dataset') and hasattr(sub_dataset, 'times'):
                        actual_sub_dataset = sub_dataset.dataset
                        actual_local_idx = local_idx % len(actual_sub_dataset)
                    else:
                        actual_sub_dataset = sub_dataset
                        actual_local_idx = local_idx
                    
                    # 尝试获取图片信息
                    if hasattr(actual_sub_dataset, 'data_infos') and actual_local_idx < len(actual_sub_dataset.data_infos):
                        data_info = actual_sub_dataset.data_infos[actual_local_idx]
                        if isinstance(data_info, dict) and 'filename' in data_info:
                            img_info = f" -> {data_info['filename']}"
                        elif isinstance(data_info, dict) and 'file_name' in data_info:
                            img_info = f" -> {data_info['file_name']}"
                    
                    # 如果没有data_infos，尝试其他属性
                    if not img_info and hasattr(actual_sub_dataset, 'img_ids'):
                        if actual_local_idx < len(actual_sub_dataset.img_ids):
                            img_id = actual_sub_dataset.img_ids[actual_local_idx]
                            img_info = f" -> img_id: {img_id}"
                            
                except Exception as e:
                    img_info = f" (unable to get filename: {str(e)[:50]}...)"
                
                print(f"   └─ Sample {j}: {dataset_name}[{sample['local_idx']}] (global_idx: {sample['global_idx']}){img_info}")
        
        print("\n" + "="*80)
        print(f"✅ Total generated samples: {len(batch_indices)} for {self.num_batches} batches")
        print("="*80 + "\n")
