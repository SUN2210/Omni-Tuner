...
class BBoxHead(nn.Module):
    ...
    def forward(self, x):
        if self.with_avg_pool:
            x = self.avg_pool(x)
        x = x.view(x.size(0), -1)
        # store for downstream prototype alignment
        self._cached_features = x
        cls_score = self.fc_cls(x) if self.with_cls else None
        bbox_pred = self.fc_reg(x) if self.with_reg else None
        return cls_score, bbox_pred

    def pop_cached_features(self):
        feats = self._cached_features
        self._cached_features = None
        return feats
    ...
    def loss(self,
        ...
        if bbox_pred is not None and 'pos_inds' in locals():
            if pos_inds.any() and self._cached_features is not None:
                self.prototype_pos_cache = (
                    self._cached_features[pos_inds], labels[pos_inds])
            else:
                self.prototype_pos_cache = None
        self._cached_features = None
        return losses
