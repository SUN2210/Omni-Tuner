...
class AnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    def _get_bboxes(self,
        ...
        if nms_pre is None:
            nms_pre = -1
        if isinstance(nms_pre, torch.Tensor):
            nms_pre_scalar = int(nms_pre.item())
        elif isinstance(nms_pre, (list, tuple)):
            flattened = []
            for val in nms_pre:
                if isinstance(val, torch.Tensor):
                    flattened.append(int(val.item()))
                elif isinstance(val, np.ndarray):
                    flattened.append(int(val.max()))
                else:
                    flattened.append(int(val))
            nms_pre_scalar = max(flattened) if flattened else -1
        elif isinstance(nms_pre, np.ndarray):
            nms_pre_scalar = int(nms_pre.max()) if nms_pre.size > 1 else int(
                nms_pre.item())
        else:
            nms_pre_scalar = int(nms_pre)
        nms_pre_tensor = mlvl_cls_scores[0].new_tensor(
            nms_pre_scalar, dtype=torch.long)
